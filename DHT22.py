...
class sensor:
    ...
    def __init__(self, pi, gpio, LED=None, power=None):
        ...
        self.cb = pi.callback(gpio, pigpio.EITHER_EDGE, self._callback)
    ...
    def update_state(self):
        #if self.tov is None or time.time() >= self.tov + POLL_INTERVAL:
        staleness = self.staleness()
        if staleness == INVALID_VALUE or staleness >= POLL_INTERVAL:
            # Trigger another reading
            self.trigger()

            # Wait for a complete reading or error
            while self.reading:
                time.sleep(0.2)
        else:
            # Return immediately and let us return cached data from previous read
            pass
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # time.sleep(next_reading -
        #            time.time())  # Overall INTERVAL second polling.
